...
\paragraph{Datasets.}
Main results are reported on GSM8K~\cite{cobbe2021gsm8k} test (1,319 problems). Transfer results are reported on BoolQ~\cite{clark2019boolq}. In the current implementation, requesting \texttt{split=test} maps to the BoolQ validation split; therefore, the BoolQ numbers reported here should be interpreted as validation results rather than hidden-test results.
...
Two properties are important for routing. First, the trained adapter is much stronger than the base model at low budgets. Second, accuracy is not monotone in budget: Acc@2 and Acc@3 exceed Acc@4. This non-monotonicity creates room for routing to improve both efficiency and accuracy.
...
Oracle reference & 0.8010$\pm$0.0259 (273$\pm$20, 1.80$\pm$0.08) & 0.8010$\pm$0.0259 (273$\pm$20, 1.80$\pm$0.08) & 0.8010$\pm$0.0259 (273$\pm$20, 1.80$\pm$0.08) & 0.8010$\pm$0.0259 (273$\pm$20, 1.80$\pm$0.08) \\
\bottomrule
\end{tabularx}
\caption{GSM8K test performance across compute tiers. Each cell reports accuracy, with mean allocated tokens and mean stopping steps in parentheses. Values are mean$\pm$std over three split seeds. The oracle row is a budget-agnostic reference shown in each column for ease of comparison.}
...
Oracle reference & 0.6133$\pm$0.0503 (554$\pm$35, 3.08$\pm$0.13) & -- & -- & -- \\
\bottomrule
\end{tabularx}
\caption{BoolQ validation performance across compute tiers. Each cell reports accuracy, with mean allocated tokens and mean stopping steps in parentheses. Values are mean$\pm$std over three split seeds. The oracle row is a single reference point rather than a tier-specific budget-matched result.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{router_pareto_all_fixed.pdf}
...
The empirical study uses a single 1.5B backbone, four discrete budgets, and three split seeds. The transfer experiment also omits target-task fine-tuning. Broader conclusions about scaling, architecture dependence, and cross-domain robustness will require a larger experimental sweep.

% ======================================================================
\section{Reproducibility}
The experimental pipeline is script-driven and reproducible from a single entry point. The workflow trains the QLoRA adapter, runs anytime evaluation to obtain per-step predictions, builds per-seed routing splits, fits per-seed Platt calibrators, evaluates compute-matched routers, and generates tables and figures. The paper's main numeric results are taken directly from the generated artifacts rather than being copied manually from ad hoc runs.
