...
from eval_depth_router import read_jsonl, evaluate


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--dev", required=True, help="Dev JSONL trajectories")
    ...
    for th in thresholds:
        ...
        grid.append({"threshold": float(th), "acc": res["acc"], "mean_steps": res["mean_steps"], "mean_tokens": res["mean_tokens"]})

    targets = [float(x.strip()) for x in args.target_mean_steps.split(",") if x.strip()]
    for B in targets:
        best = min(grid, key=lambda r: abs(r["mean_steps"] - B))
        print(json.dumps({"target_mean_steps": B, "best_threshold": best["threshold"], "mean_steps": best["mean_steps"], "acc": best["acc"]}))
