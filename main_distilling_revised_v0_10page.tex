...
A compute-matched evaluation protocol is used: for each tier (B1--B4), adaptive routers are tuned to match the fixed-depth baseline's expected allocated-token compute, using a deterministic two-point mixture when exact matching is not attainable. On GSM8K test over three split seeds, confidence-threshold routing improves accuracy at B2 from 0.6603$\pm$0.0244 to 0.6937$\pm$0.0063 at similar mean allocated tokens (256 vs.\ 260), while stability-based routing improves accuracy at B3 from 0.6540$\pm$0.0311 to 0.6844$\pm$0.0305 (480 vs.\ 484). An oracle stopping reference reaches 0.8010$\pm$0.0259 at 272.9$\pm$19.5 tokens, indicating substantial headroom. Transfer to BoolQ without further fine-tuning is markedly weaker, and self-reported confidence is less effective than on GSM8K. The GSM8K anytime-quality rows and GSM8K routing rows are reported from two explicit canonical artifact families documented by a row-match audit, rather than one row-identical prediction bundle.
\end{abstract}
...
This scope is intentionally narrow: the paper targets dependable adaptive compute allocation for autonomous inference loops under budget constraints, not broader trusted-AI governance, security, privacy, or safety-certification claims.
...
Before routing, the quality of the underlying trajectories is measured. Table~\ref{tab:anytime_gsm8k} summarizes the unadapted base model and the trained adapter at each budget $t$ on GSM8K test. For audit clarity, these anytime rows are computed from \texttt{results\_abl/preds\_base.jsonl} and \texttt{results\_abl/preds\_main\_adapter.jsonl}; they are not the same row bundle used by the Option-B GSM8K router evaluation.
...
\bottomrule
\end{tabular}
\caption{Anytime accuracy on GSM8K test at each budget $t$ under strict answer matching, computed from \texttt{results\_abl/preds\_base.jsonl} and \texttt{results\_abl/preds\_main\_adapter.jsonl} (not from the Option-B router source bundle).}
\label{tab:anytime_gsm8k}
\end{table}

Two properties are important for routing. First, the trained adapter is much stronger than the base model at low budgets. Second, accuracy is not monotone in budget: Acc@2 and Acc@3 exceed Acc@4. This non-monotonicity creates room for routing to improve both efficiency and accuracy. Importantly, the anytime rows in Table~\ref{tab:anytime_gsm8k} come from the \texttt{results\_abl/*} family, while Table~\ref{tab:main_results_allbudgets} is computed from \texttt{results/preds\_student\_full.jsonl} $\rightarrow$ \texttt{data/router\_splits\_seeds/manifest.json} $\rightarrow$ \texttt{artifacts/router\_optionB/*.csv}; \texttt{artifacts/paper/tables/provenance\_rowmatch\_audit.csv} documents that these families are not row-identical.

\subsection{Compute-Matched Routing on GSM8K}
Table~\ref{tab:main_results_allbudgets} reports compute-matched routing results on GSM8K test using the Option-B protocol from Section~\ref{sec:optionb}. This routing table is sourced from the canonical Option-B family rooted at \texttt{results/preds\_student\_full.jsonl}, not from the \texttt{results\_abl/*} anytime-analysis family used in Table~\ref{tab:anytime_gsm8k}.

\input{artifacts/paper/tables/router_table.tex}

At B2, confidence routing yields the largest gain over fixed depth (0.6937 vs.\ 0.6603) at nearly identical compute. At B3, stability routing performs best among the non-oracle policies (0.6844 vs.\ 0.6540). A lightweight paired split-seed bootstrap from the bundled per-seed Option-B table gives a mean delta of +0.0333 accuracy for confidence B2 vs fixed B2 (95\% CI [0.0114, 0.0519]) and +0.0304 for stability B3 vs fixed B3 (95\% CI [0.0297, 0.0313]); this support is intentionally narrow (split seeds only, $n{=}3$) and does not estimate retraining variance. The random-matched baseline remains close to fixed depth, suggesting that the gains arise from using trajectory information rather than merely changing the stopping histogram. The oracle reference indicates large remaining headroom: many examples are solvable earlier than B4, but the available signals do not yet identify those cases reliably.
...
\input{artifacts/paper/tables/router_table_boolq.tex}
...
The compute-matched protocol uses allocated tokens (\texttt{max\_new\_tokens}) as a stable proxy for inference cost in online control. Actual generated tokens are also available in the evaluation pipeline, but they are not used when constructing routing costs. Conclusions should therefore be interpreted as matching \emph{allocated} rather than realized compute.

\paragraph{Scope limitations.}
The empirical study uses a single 1.5B backbone, four discrete budgets, and three split seeds. Reported variability is split-seed variability from router dev/test partitioning only, and does not include retraining/model-seed variance for the student or router feature pipeline. The added paired split-seed bootstrap artifact provides only a small-scope uncertainty check on routed-vs-fixed deltas and should be interpreted within the same limitation. The bundled teacher trajectory file (\texttt{data/anytime\_gsm8k\_train\_v2.jsonl}) also shows nontrivial checkpoint noise: 460/7,473 trajectories (6.16\%) contain at least one answer flip across $t{=}1\ldots4$, and 47/7,473 (0.63\%) contain a correct$\rightarrow$incorrect regression. The transfer experiment also omits target-task fine-tuning and is only partially reproducible from this snapshot because some BoolQ-side inputs are Git LFS/externally dependent. Broader conclusions about scaling, architecture dependence, and cross-domain robustness will require a larger experimental sweep.

% ======================================================================
\section{Reproducibility}
The repository is available at \url{https://github.com/altieriumb7/adaptive-anytime-llm-routing.git}. The experimental pipeline is script-driven and partially reproducible from the shipped repository. Tables and figures can be regenerated from bundled prediction/router artifacts, and paper claims are artifact-level claims tied to those canonical files rather than a full raw-to-paper rerun guarantee. For GSM8K, this snapshot contains two explicit canonical families: (i) \texttt{results/preds\_student\_full.jsonl} with split manifests and Option-B CSV exports for routing tables, and (ii) \texttt{results\_abl/preds\_main\_adapter.jsonl} plus \texttt{results\_abl/preds\_base.jsonl} for anytime/calibration analyses; the row-match audit CSV records that these are not one row-identical bundle. Split-seed router variability can be recomputed from bundled manifests, and a lightweight paired split-seed bootstrap summary is exported to \texttt{artifacts/paper/tables/router\_paired\_bootstrap\_gsm8k.csv}; model-seed retraining variability is not evaluated in the released artifact. Full end-to-end regeneration (including trajectory generation and split-level BoolQ reruns) depends on external datasets, model checkpoints, and Git LFS objects not bundled directly in this snapshot; therefore BoolQ claims in this release are artifact-level unless transfer split payloads are materialized.

% ======================================================================
\section{Conclusion}
Budget-conditioned anytime inference provides a useful setting for studying adaptive test-time compute in LLMs. Under matched expected compute, lightweight depth routers improve intermediate-budget performance on GSM8K relative to fixed-depth baselines. Confidence-threshold routing is strongest at B2, while stability-based routing is strongest at B3. Transfer to BoolQ without further fine-tuning is substantially less favorable, especially for confidence-based stopping, which suggests that confidence signals learned on math reasoning do not directly generalize across tasks. The oracle reference indicates considerable room for stronger stopping criteria. Overall, the strongest takeaway is a focused systems result on budget-aware routing behavior rather than a broad cross-domain trust guarantee.
