...
\bottomrule
\end{tabularx}
\caption{BoolQ validation performance across compute tiers (accuracy; mean tokens and mean steps in parentheses). Adaptive policies are tuned on dev to match the fixed tier's mean token compute in expectation; values are mean$\pm$std over three split seeds.}
